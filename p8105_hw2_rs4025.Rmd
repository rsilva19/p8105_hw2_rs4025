...
The 'snp' dataset initially contained `r nrow(snp)` observations of two variables: date, and closing value of S&P index (labeled 'close'). We tidied the date column to contain variables 'year' and 'month' and arranged the observations by year and month. The dataset now contains `r ncol(snp)` variables and ranges from the years `r min(pull(snp, year))` to `r max(pull(snp, year))`. 
...
We merged all three of the above datasets into the dataset 'final_join'. The final dataset contains `r nrow(final_join)` observations of `r ncol(final_join)` variables which includes observations from `r min(pull(final_join, year))` to `r max(pull(unemployment, year))`. We are most interested in the variables 'year', 'month', 'president', 'logical', 'close', and 'percent_unemployed', all previously described. Due to the nature of joining data, we are left with some NA's, most coming from specific years when that data might not have been collected. 
